...
# Test 4: strict credential enforcement should fail if required environment variables are missing
set +e
(
  cd "$REPO_OK"
  CREDENTIAL_ENFORCEMENT=true REQUIRED_CREDENTIAL_ENVS="OPENAI_API_KEY,AZURE_OPENAI_API_KEY" \
    STRICT_MODE=true REPORT_FILE="$TMP_DIR/report-creds-fail.txt" "$SCRIPT"
)
creds_fail_exit_code=$?
set -e
if [[ "$creds_fail_exit_code" -eq 0 ]]; then
  echo "Expected strict mode to fail when required credentials are missing"
  exit 1
fi
assert_contains "FAIL [global]: CREDENTIAL_AVAILABLE OPENAI_API_KEY (missing env var)" "$TMP_DIR/report-creds-fail.txt"
assert_contains "FAIL [global]: CREDENTIAL_AVAILABLE AZURE_OPENAI_API_KEY (missing env var)" "$TMP_DIR/report-creds-fail.txt"

# Test 5: credential enforcement should pass when required credentials are present
(
  cd "$REPO_OK"
  OPENAI_API_KEY="dummy-openai-key" AZURE_OPENAI_API_KEY="dummy-azure-key" \
    CREDENTIAL_ENFORCEMENT=true REQUIRED_CREDENTIAL_ENVS="OPENAI_API_KEY,AZURE_OPENAI_API_KEY" \
    STRICT_MODE=true REPORT_FILE="$TMP_DIR/report-creds-pass.txt" "$SCRIPT"
)
assert_contains "PASS [global]: CREDENTIAL_AVAILABLE OPENAI_API_KEY" "$TMP_DIR/report-creds-pass.txt"
assert_contains "PASS [global]: CREDENTIAL_AVAILABLE AZURE_OPENAI_API_KEY" "$TMP_DIR/report-creds-pass.txt"
assert_contains "Total failures: 0" "$TMP_DIR/report-creds-pass.txt"
